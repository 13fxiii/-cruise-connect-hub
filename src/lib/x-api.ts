...
export async function getUserXAccessToken(userId: string): Promise<string> {
  ...
  const isExpired = expiresAtMs && Number.isFinite(expiresAtMs) ? Date.now() > expiresAtMs - 60_000 : false;
  ...
  const { error: updateError } = await (supabaseAdmin as any)
    ...
    .eq("user_id", userId);
  if (updateError) {
    throw new Error(`Failed to persist refreshed X token: ${updateError.message}`);
  }
