...
function getOAuthCookie(request: NextRequest, name: string) { return request.cookies.get(`__Host-${name}`)?.value; }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const proxyEmail = `x_${xUser.id}@cruise-connect.app`; const displayName = xUser.name || xUser.username;
    const avatarRaw = xUser.profileImageUrl || xUser.profile_image_url || ''; const avatarUrl = String(avatarRaw).replace('_normal', '_400x400');
    const { data: existingUserData, error: lookupError } = await supabase.auth.admin.getUserByEmail(proxyEmail);
    if (lookupError && !lookupError.message.toLowerCase().includes('not found')) console.error('X user lookup error:', lookupError);
    let userId = existingUserData?.user?.id; let isNewUser = false;
    if (!userId) {
      isNewUser = true;
    ...
    const response = NextResponse.redirect(`${appUrl}${isNewUser ? '/onboarding' : redirectTo}`); const { access_token, refresh_token } = sessionData.session;
    const sessionCookie = { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax' as const, maxAge: 60 * 60 * 24 * 30, path: '/' };
    response.cookies.set('__Host-sb-access-token', access_token, sessionCookie); response.cookies.set('__Host-sb-refresh-token', refresh_token, sessionCookie);
    response.cookies.delete('__Host-x_code_verifier'); response.cookies.delete('__Host-x_oauth_state'); response.cookies.delete('__Host-x_oauth_redirect_to'); response.headers.set('Cache-Control', 'no-store');
    return response;
  } catch (err) { console.error('X OAuth callback error:', err); return NextResponse.redirect(`${appUrl}/auth/login?error=x_unknown`); }
}
