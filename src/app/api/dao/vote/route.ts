...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: proposal } = await supabaseAdmin
      .from('dao_proposals').select('*').eq('id', proposal_id).single();
    ...
    const { data: allVotes } = await supabaseAdmin
      .from('dao_votes').select('vote').eq('proposal_id', proposal_id);
    ...
    if (total >= proposal.min_votes) {
      ...
      if (passRate >= proposal.pass_threshold) newStatus = 'passed';
    }

    await supabaseAdmin.from('dao_proposals').update({
