...
import { Menu, X, Zap, Rss, Megaphone, Mic, Gamepad2, Wallet, User } from "lucide-react";
    ...
    <nav className="sticky top-0 z-50 bg-black/95 backdrop-blur-md border-b border-zinc-800">
      <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-black text-sm"><span className="text-white">C&C </span><span className="text-yellow-400">Hub</span></span>
        </Link>
      ...
      {open && (
        <div className="md:hidden bg-black border-t border-zinc-800 px-4 py-4 flex flex-col gap-1">
          {NAV_LINKS.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium ${path === href ? "bg-yellow-400/10 text-yellow-400" : "text-zinc-300 hover:text-white"}`}>
              <Icon size={16} /> {label}
            </Link>
          ))}
          <div className="border-t border-zinc-800 mt-2 pt-3">
            <Link href="/auth/login" className="block bg-yellow-400 text-black text-sm font-black py-2.5 rounded-full text-center">Join Hub</Link>
          </div>
        </div>
      )}
    </nav>
